...
const getCurrentApplicationForm = async (req, res) => {
  try {
    ...
    if (!applicationForm) {
      ...
      console.log("Created application form with default fields:", applicationForm._id.toString());
    } else if (!Array.isArray(applicationForm.fields) || applicationForm.fields.length === 0) {
      applicationForm.fields = DEFAULT_FIELDS;
      await applicationForm.save();
      console.log("Seeded default fields into existing application form:", applicationForm._id.toString());
    }
    ...
    const mergeWithDefaultFields = (existingFields = []) => {
      ...
      const customOnly = (Array.isArray(existingFields) ? existingFields : []).filter(
    ...
    const formObj = applicationForm.toObject ? applicationForm.toObject() : applicationForm;
